# Don't Remove Credit Tg - @SAMEERJI91
# Ask Doubt on telegram @SAMEERJI91
...
async def restart_handler(_, m: Message):
    await m.reply_text("**𝗦𝗧𝗢𝗣𝗣𝗘𝗗**🚦", True)
...
async def edit_txt(client, message: Message):
    ...
    try:
        await message.reply_document(
            ...
            caption="📥**𝗘𝗱𝗶𝘁𝗲𝗱 𝗕𝘆 ➤ ꜱᴀᴍᴇᴇʀ ʙʜʏʏᴀ**"
        )
    except Exception as e:
...
async def help_command(client: Client, msg: Message):
    help_text = (
        "`/start` - Start the bot⚡\n\n"
        "`/ASP` - Download and upload files (sudo)🎬\n\n"
...
async def upload(bot: Client, m: Message):
    ...
    await editable.edit("📛 𝗘𝗻𝘁𝗲𝗿 𝗬𝗼𝘂𝗿 𝗡𝗮𝗺𝗲 📛\n\n🐥 𝗦𝗲𝗻𝗱 `1` 𝗙𝗼𝗿 𝗨𝘀𝗲 𝗗𝗲𝗳𝗮𝘂𝗹𝘁 🐥Egˏˋ°•*⁀➷ `𝗦𝗔𝗠𝗘𝗘𝗥 𝗕𝗛𝗬𝗬𝗔`")
    ...
    await m.reply_text(f"`✨𝗕𝗔𝗧𝗖𝗛 𝗦𝗨𝗠𝗠𝗔𝗥𝗬✨\n\n"
                       ...
                       f"▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n"
                       f"✅𝗦𝗧𝗔𝗧𝗨𝗦 » 𝗖𝗢𝗠𝗣𝗟𝗘𝗧𝗘𝗗`")
    await m.reply_text(f"<pre><code>📥𝗘𝗫𝗧𝗥𝗔𝗖𝗧𝗘𝗗 𝗕𝗬 ➤『{CR}』</code></pre>")
